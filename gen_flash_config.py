...
def main(output_path: pathlib.Path, skus: str = typer.Argument("")):
    if "," in skus:
        skus = skus.split(",")
    else:
        skus = [skus]
    ...
    if len(flashes["nvm"]) == 0:
        print("Unknown flash chip(s)")
    ...
    flash_size = all_match(flashes["nvm"], "total_size")
    ...
    template = Template(read_text("templates", "flash_config.c.jinja"))
